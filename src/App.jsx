...
function EditorApp() {
  ...
  const generateEmailHTMLV1 = () => {
    ...
    const sH1 = data.sizeH1 || 46;
    const sH2 = data.sizeH2 || 34;
    const sBody = data.sizeBody || 17;
    const sEye = data.sizeEyebrow || 11;
    ...
    const goldBtn = (label) =>
      `<div style="display:inline-block;padding:12px 32px;background:${gold};color:${plumDeep};font-size:13px;letter-spacing:2.5px;text-transform:uppercase;font-weight:800;font-family:'Inter Tight',sans-serif;">${label}</div>`;
    ...
    const eventosHTML = !hidden.eventos && data.eventos && data.eventos.length > 0
      ...
      <div style="background:${plumDeep};color:#fff;">
        <div style="padding:32px 36px 24px;text-align:center;">
          <div style="font-size:11px;letter-spacing:4px;color:${gold};font-weight:800;text-transform:uppercase;margin-bottom:10px;">· Reclutamiento Semanal ·</div>
        ...
        <div style="padding:24px 36px 32px;text-align:center;">
          ...
          <p style="font-size:15px;line-height:1.75;color:#d6cae8;margin:0 auto 20px;max-width:380px;">${ev.desc || ''}</p>
    ...
    const capacitacionesHTML = !hidden.capacitacion && data.capacitaciones && data.capacitaciones.length > 0
      ...
      <div style="background:${creamSoft};padding:36px 36px 32px;text-align:center;">
        ...
        <p style="font-size:15px;line-height:1.75;color:#4a3f54;margin:0 auto 20px;max-width:420px;">${c.desc || ''}</p>
...
<center>
<table width="680" cellpadding="0" cellspacing="0" border="0" style="max-width:680px;margin:0 auto;background:${cream};font-family:'Plus Jakarta Sans',system-ui,sans-serif;">
  ...
  <tr><td style="background:${cream};padding:22px 0 16px;text-align:center;border-bottom:1px solid ${plum}15;">
    ...
    <div style="margin-top:10px;font-size:11px;letter-spacing:3.5px;color:${plum};font-family:'Fraunces',serif;font-style:italic;opacity:0.7;">${data.weekNum || ''} · ${data.weekMonth || ''}</div>
  </td></tr>
  ...
  <tr><td style="padding:48px 36px 40px;background:${plumDeep};color:#a89bb8;text-align:center;font-size:10.5px;line-height:1.8;">
    ...
    <div style="font-size:13px;color:#c0b5d0;line-height:1.7;">Recibes este correo porque eres parte de la ${data.brandName || 'Sensation Family'}.</div>
    <div style="font-size:13px;color:#c0b5d0;line-height:1.7;margin-bottom:24px;">¿Ya no quieres recibir nuestros boletines? <a href="#" style="color:${gold};text-decoration:underline;">Darme de baja</a></div>
        ...
        <aside style={{
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px 8px' }}>
            ...
            <Section id="estilos" icon="A" title="Estilos del texto" badge="OPCIONAL" openState={openState} setOpenState={setOpenState} tk={tk}>
              ...
              <SizeField k="sizeH1" label="Título Hero" min={24} max={64} unit="px" data={data} set={set} tk={tk} />
              <SizeField k="sizeH2" label="Subtítulos de sección" min={18} max={44} unit="px" data={data} set={set} tk={tk} />
              <SizeField k="sizeBody" label="Texto de párrafo" min={12} max={22} unit="px" data={data} set={set} tk={tk} />
              <SizeField k="sizeEyebrow" label="Etiquetas (eyebrow)" min={9} max={16} unit="px" data={data} set={set} tk={tk} />
            </Section>
