...
export function generateEmailHTMLV3(data, hiddenSections) {
  ...
  const eye = (l, c = gold, mb = 14) =>
    `<p style="margin:0 0 ${mb}px;font-family:${ff};font-size:13px;letter-spacing:4px;color:${c};text-transform:uppercase;font-weight:600;">${l}</p>`;
  const gBtn = (l, h = '#') =>
    `<a href="${h}" style="display:inline-block;background:${gradGold};color:${plumDark};font-family:${ff};font-size:13px;letter-spacing:3px;text-transform:uppercase;font-weight:800;padding:16px 32px;border-radius:50px;text-decoration:none;box-shadow:0 4px 18px rgba(201,162,76,0.45);">${l}</a>`;
  const pBtn = (l, h = '#') =>
    `<a href="${h}" style="display:inline-block;background:${gradPlum};color:#fff;font-family:${ff};font-size:13px;letter-spacing:3px;text-transform:uppercase;font-weight:800;padding:16px 32px;border-radius:50px;text-decoration:none;">${l}</a>`;
...
<style>body,table,td,a{font-family:'Jost',Arial,Helvetica,sans-serif;}img{border:0;display:block;}body{margin:0;padding:0;background:#FAF6EE;}a{color:${gold};text-decoration:none;}@media(max-width:600px){.container{width:100%!important;border-radius:0!important;}.img-full{width:100%!important;height:auto!important;}}</style>
</head>
...
<table role="presentation" class="container" width="680" cellspacing="0" cellpadding="0" border="0" style="width:680px;max-width:680px;background:#fff;border-radius:14px;overflow:hidden;">
